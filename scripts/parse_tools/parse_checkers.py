...
def check_local_name(test_val, prop_dict, error, max_len=0):
    ...
    A constant is only valid if <prop_dict> is not None, the 'constant'
    property is present and True, and the 'type' property matches the
    type of <test_val>.
    if <error> is True, raise an Exception if <test_val> is not valid.
    >>> check_local_name("hi_mom", None, False)
    'hi_mom'
    >>> check_local_name('122', {'constant':True,'type':'integer'}, False)
    '122'
    >>> check_local_name('122', None, False)

    >>> check_local_name('122', {}, False)

    >>> check_local_name('122', {'constant':False,'type':'integer'}, False)

    >>> check_local_name('122', {'constant':True,'type':'real'}, False)

    >>> check_local_name('-122.e4', {'constant':True,'type':'real'}, False)
    '-122.e4'
    >>> check_local_name('-122.', {'constant':True,'type':'real','kind':'kp'}, False)

    >>> check_local_name('-122._kp', {'constant':True,'type':'real','kind':'kp'}, False)
    '-122._kp'
    """
    valid_val = None
    ...
    if (prop_dict is not None) and ('kind' in prop_dict):
        ...
        kind = ""
    # End if
    if constant and (len(vtype) > 0) and check_fortran_literal(test_val,
                                                               vtype, kind):
        valid_val = test_val
    # End if
