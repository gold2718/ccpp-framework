    ...
    def write_inspection_routines(self, ofile):
        ...
        for suite in self.suites:
            ...
            if have_elems:
                ...
                numvars = len(input_vars[0].union(input_vars[2], inout_vars[0],
                                                  inout_vars[2], output_vars[0],
                                                  output_vars[2]))
                ofile.write("num_vars = {}".format(numvars), 5)
                ofile.write("else", 4)
            # end if
            numvars = len(input_vars[0].union(input_vars[1], inout_vars[0],
                                              inout_vars[1], output_vars[0],
                                              output_vars[1]))
            ...
            if have_elems:
                ofile.write("if (struct_elements_use) then", 4)
                numvars = len(inout_vars[0].union(inout_vars[2], output_vars[0],
                                                  output_vars[2]))
                ofile.write("num_vars = {}".format(numvars), 5)
                ofile.write("else", 4)
            # end if
            numvars = len(inout_vars[0].union(inout_vars[1], output_vars[0],
                                              output_vars[1]))
            ofile.write("num_vars = {}".format(numvars), 5 if have_elems else 4)
            if have_elems:
                ofile.write("end if", 4)
            ...
            elem_list = sorted(inout_vars[2].difference(elem_written_set))
            elem_written_set = elem_written_set.union(inout_vars[2])
            leaf_list = sorted(inout_vars[1].difference(leaf_written_set))
            leaf_written_set = leaf_written_set.union(inout_vars[1])
            ...
            leaf_list = sorted(input_vars[0].difference(leaf_written_set))
            leaf_written_set = leaf_written_set.union(input_vars[0])
            elem_written_set = elem_written_set.union(input_vars[0])
            elem_list = sorted(input_vars[2].difference(elem_written_set))
