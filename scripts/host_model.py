...
from parse_tools import context_string, check_fortran_intrinsic
from parse_tools import FORTRAN_ID, FORTRAN_SCALAR_REF_RE
...
class HostModel(VarDictionary):
        ...
        for header in [h for h in meta_headers if h.header_type != 'ddt']:
            ...
            if header.header_type == 'module':
                ...
                for var in header.variable_list():
                    ...
                    if var.is_ddt():
                        self.__ddt_lib.collect_ddt_fields(self.__ddt_dict,
                                                          var, logger=None)
                    # End if
                # End for
                ...
                for hvar in loop_vars:
                    ...
                    if std_name in self.__loop_vars:
    ...
    def variable_locations(self):
        ...
        for name in lnames:
            ...
            if (module is not None) and (len(module) > 0) and used:
